...
, index: function() {
    ...
    for (var i = 0, l = this.modelViews.length, mv; i < l; i++) {
      ...
      , listURL: URLConf.reverse(mv.urlName('list'))
      })
    }
...
, list: function() {
    ...
    return this.display([this.namespace + ':admin:list', 'admin:list']
      , { items: items
        ...
        , addURL: URLConf.reverse(this.urlName('add'))
        }
      )
  }
...
, detail: function(id) {
    ...
    return this.display([this.namespace + ':admin:detail', 'admin:detail']
      , { item: item
        , editURL: URLConf.reverse(this.urlName('edit'), [id])
        , deleteURL: URLConf.reverse(this.urlName('delete'), [id])
        }
      )
  }
...
, add: function(data) {
    ...
    if (data) {
      ...
      if (form.isValid()) {
        ...
        return this.redirect(
          URLConf.reverse(this.urlName('detail'), [instance.id])
        )
      }
      else if (!server) {
    ...
    return this.display([this.namespace + ':admin:add', 'admin:add']
      , { form: form
        , submitURL: URLConf.reverse(this.urlName('add'))
        , cancelURL: URLConf.reverse(this.urlName('list'))
        }
      )
  }
...
, edit: function(id, data) {
    ...
    if (data) {
      ...
      if (form.isValid()) {
        ...
        return this.redirect(
          URLConf.reverse(this.urlName('detail'), [id])
        )
      }
      else if (!server) {
        // Look up the <tbody> containing the form rows and replace its contents
    ...
    return this.display([this.namespace + ':admin:edit', 'admin:edit']
      , { item: item
        , form: form
        , submitURL: URLConf.reverse(this.urlName('edit'), [id])
        , cancelURL: URLConf.reverse(this.urlName('detail'), [id])
        }
      )
  }
...
, delete_: function(id, data) {
    ...
    if (data) {
      ...
      return this.redirect(
        URLConf.reverse(this.urlName('list'))
      )
    }
    else {
      return this.display([this.namespace + ':admin:delete', 'admin:delete']
        , { item: item
          , submitURL: URLConf.reverse(this.urlName('delete'), [id])
          , cancelURL: URLConf.reverse(this.urlName('detail'), [id])
          }
        )
    }
